...
**แก้ไขภายหลัง (ตรวจพบตอนรีวิวจบสาย):** ที่จุดนี้ของแผน `tsc --noEmit` **ยังไม่ผ่านจริง** —
`queue_entries.bed_id_2` และ `services.splits_room` ยังไม่มีอยู่ในฐานข้อมูล (มาใน Task 3
Step 3 ที่ apply migration) ชนิด `Tables<"queue_entries">`/`Tables<"services">` ที่ regenerate
จาก `src/types/database.ts` จึงยังไม่รู้จักสองคอลัมน์นี้จนกว่า Task 3 จะเสร็จ โค้ดหน้าจอ (Task 4, 5)
ที่อ่าน `entry.bed_id_2`/`service.splits_room` ผ่าน type จากฐานข้อมูลตรงๆ จะคอมไพล์ไม่ผ่านถ้ารันก่อน
Task 3 — schema ต้องลงก่อนถึงจะเรียก `tsc` แล้วผ่านทั้งโปรเจกต์ได้จริง
...
Expected: **ทุกแถว `result = PASS`**

**แก้ไขภายหลัง (ตัดสินใจกลางทางระหว่างทำ):** `bed_double_booked` **ไม่กลายเป็น 0** — ค่าที่ถูกต้อง
คือ **1 ถาวร** (known exception เหมือน `credit_used_exceeds_net`/`credit_used_without_customer`)
เพราะคู่ 9 ส.ค. 2026 (เอ็ม เมธี/กอล์ฟฟี่) เกิดขึ้น**ก่อน**คอลัมน์ `bed_id_2` จะมีอยู่จริง จึงไม่มีทาง
บันทึกย้อนหลังว่าเอ็ม เมธีย้ายไปห้องไหนได้เลย — backfill เท่ากับกุข้อมูลขึ้นมาเองโดยไม่มีใครยืนยัน
ได้ ซึ่งเป็นสิ่งที่ Global Constraints ห้ามไว้ชัดเจน (**ห้ามแก้ข้อมูลคิวคู่ที่ `bed_double_booked` ฟ้อง**)
ไม่ใช่ว่าข้อตรวจนี้ยังไม่ถูกแก้ — ดูเหตุผลเต็มใน `supabase/reconciliation.sql` ตรงคอมเมนต์เหนือ
`('bed_double_booked', 1)` และใน design doc

ถ้าค่าที่ได้เป็น 1 → ถือว่า PASS ตามที่ตั้งใจ **ถ้าขึ้นมากกว่า 1 = มีคู่ใหม่ที่ต้องสืบทันที** (การ์ดที่
สร้างหลังฟีเจอร์นี้ขึ้นระบุ `bed_id_2` ได้แล้ว ไม่ควรมีข้อยกเว้นใหม่งอกขึ้นมาอีก) — query ดูว่าคู่ไหน
ที่ฟ้อง **อย่าแก้ข้อมูลเพื่อให้ผ่าน** — รายงานกลับมา
...
Expected: **ทุกข้อ PASS** — `bed_double_booked` ที่เคย FAIL (นับเป็น 2 คู่ขึ้นไปไม่ได้) ต้องนิ่งที่
ค่าที่ถูกต้องคือ **1** (known exception ถาวรของคู่ 9 ส.ค. 2026 ที่เกิดก่อนคอลัมน์ `bed_id_2` — ดู
คอมเมนต์ในสเปกและ Task 6) **ไม่ใช่ 0** — ถ้าเห็น 0 ให้ตรวจว่า known exception ในสูตรยังอยู่ครบไหม
ก่อนดีใจ เพราะแปลว่าตัวข้อตรวจเองอาจมองไม่เห็นคู่นั้นด้วยเหตุอื่น ไม่ใช่สัญญาณว่าทำถูก
...
และแจ้งว่าข้อตรวจ `bed_double_booked` ที่ค้างมาตั้งแต่ 9 ส.ค. **นิ่งที่ 1 ถาวรแล้ว (ไม่ใช่ 0)**
โดยไม่ได้แก้ข้อมูลเลย เพราะคิวคู่นั้นไม่เคยผิด — ระบบต่างหากที่บันทึกความจริงไม่ครบ และคู่นั้นเกิด
ก่อนคอลัมน์ `bed_id_2` จะมีอยู่จริง เลยไม่มีทางย้อนไปบันทึกห้องที่สองให้ถูกต้องได้ (backfill ต้องกุ
ข้อมูลขึ้นเอง) ถ้าวันหน้าเลขนี้ขึ้นมากกว่า 1 ค่อยสืบว่าเป็นคู่ใหม่จริงหรือเปล่า
